...
@ray.remote
def bubble_sort_distributed(A):
    bubble_sort(A)


# Normal Python in a single process
def run_local_bubble_sort(A):
    bubble_sort(A)


A = [randint(1, 10**10) for _ in range(10**3)]
B = A.copy()
print('bubble sort local run')
cProfile.run(f"run_local_bubble_sort({A})")


# Distributed on a Ray cluster
def run_remote_bubble_sort(A):
    ray.get(bubble_sort_distributed.remote(A))


print('bubble sort remote run')
cProfile.run(f"run_remote_bubble_sort({B})")
