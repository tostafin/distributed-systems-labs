...
# at least 100 kB = 100 000 B = 800 000 b = 8 * 10^5 b < 10^6 b
size = 10**6
L = [randint(1, 10**4) for _ in range(size)]
print(getsizeof(L) // 1000)
L_obj_ref = ray.put(L)
D = {i: randint(1, 10**4) for i in range(size)}
D_obj_ref = ray.put(D)
print(getsizeof(D) // 1000)


@ray.remote
def assign_values_to_collection(C):
    for i in range(size):
        C[i] = i


print('start big list task')
cProfile.run("assign_values_to_collection.remote(L_obj_ref)")

print('start big dict task')
cProfile.run("assign_values_to_collection.remote(D_obj_ref)")
